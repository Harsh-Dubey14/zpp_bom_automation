  ...
  function (
    ...
    return Controller.extend("zppbomautomation.controller.BOMChangeItem", {
      ...
      onDelete: function () {
        ...
        MessageToast.show("Selected item(s) removed from screen. They will be deleted on Post.");
      },
      ...
      onSortStringChange: function (oEvent) {
        var oInput = oEvent.getSource();
        var oContext = oInput.getBindingContext("itemModel");
        var sValue = this._cleanSortString(
          String(oInput.getValue() || "").trim().toUpperCase()
        );
      ...
      _applySortStringSelectionsToRow: function (aSelectedZcomb) {
        ...
        MessageToast.show(aSelectedZcomb.length + " sort string(s) applied.");
      },
          ...
          for (var i = 0; i < aPayloads.length; i++) {
            ...
            if (!this._isBackendActionSuccess(oResponse)) {
              throw {
                message:
                  "Row " +
                  (aPayloads[i].BillOfMaterialItemNumber || "") +
                  " failed. " +
        ...
        return aBackendItems.map(
          function (oItem, iIndex) {
            return {
              ...
              sortString: this._cleanSortString(
                oItem.BOMItemSorter ||
                  oItem.SortString ||
                  ""
              ),
      ...
      _cleanSortString: function (sSortString) {
        sSortString = String(sSortString || "").trim().toUpperCase();

        if (/^\d{4}$/.test(sSortString)) {
          return "";
        }
