...
sap.ui.define([], function () {
  ...
  return {
    postAction: function (oModel, sRelativePath, oPayload) {
      ...
      return new Promise(function (resolve, reject) {
        getCsrfToken(sServiceUrl)
          .then(function (sToken) {
            return postWithToken(sUrl, oPayload, sToken);
          })
          .then(resolve)
          .catch(function (oError) {
            /*
             * If token became invalid/expired, clear cache and retry once.
             */
            if (oError && (oError.status === 403 || oError.status === 401)) {
              delete mCsrfTokenCache[sServiceUrl];
  ...
  function getCsrfToken(sServiceUrl) {
    return new Promise(function (resolve, reject) {
      if (mCsrfTokenCache[sServiceUrl]) {
        resolve(mCsrfTokenCache[sServiceUrl]);
        return;
      }
      ...
      jQuery.ajax({
        ...
        success: function (data, textStatus, jqXHR) {
          ...
          mCsrfTokenCache[sServiceUrl] = sToken;
          resolve(sToken);
        },
        error: function (oXHR) {
          reject({
  ...
  function postWithToken(sUrl, oPayload, sToken) {
    return new Promise(function (resolve, reject) {
      jQuery.ajax({
        url: sUrl,
        method: "POST",
